...
local filesystem = (function()
    ...
    tbl.get_files = function()
        ...
        add_search_path(call, "./csgo/antiprimo/", "metaset_configs")
        local file_names = {}
...
local backup = {}
local clantag = {
    tag = {"", "a", "an", "ant", "anti", "antip", "antipr", "antipri", "antiprim", "antiprimo", "antiprimo", "antiprimo", "antiprimo", "antiprimo", "antiprim", "antipri", "antipr", "antip", "anti", "ant", "an", "a", ""}
}
...
local configs = {
    dir = "antiprimo/",
    default_dir = "antiprimo/default.cfg",
...
local ui = {
    nav = {
        ...
        text = groups.nav:text("Welcome to antiprimo: " .. info.user),
...
screen.watermark = function(self)
    local watermark = tools.animated_text(fonts.verdana, "A N T I P R I M O . C C", ui.accent:get(), vec2_t(screen.size.x / 2, screen.size.y - 15), 1)
    watermark()
end
...
screen.indicators = function(self)
    ...
    elseif ui.visuals.indicators:get() == 3 then
        ...
        local gradient = tools.animated_text(fonts.verdanabold, 'antiprimo', ui.accent:get(), vec2_t(screen.size.x / 2 + binds.movetext, screen.size.y / 2 + self.positions[binds.style][1]), 1)
        gradient()
